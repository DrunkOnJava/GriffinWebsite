...
# Stackbit Integration Installation Script
# For Netlify-hosted, GitHub-managed websites
# Created: 2025-03-29
...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}  Stackbit Integration Installation Script  ${NC}"
echo -e "${BLUE}============================================${NC}"
echo ""
...
curl -s -o "$TEMP_DIR/setup-stackbit.js" https://raw.githubusercontent.com/yourusername/stackbit-setup/main/setup-stackbit.js || {
    # If GitHub URL fails, use the local file
    echo "Using local setup script..."
    cp ./setup-stackbit.js "$TEMP_DIR/setup-stackbit.js" || {
        echo "Error: Could not find setup script."
        
        # Create the setup script directly in the temp dir if not found
        echo "Creating setup script directly..."
        cat > "$TEMP_DIR/setup-stackbit.js" << 'EOF'
#!/usr/bin/env node
...
chmod +x "$TEMP_DIR/setup-stackbit.js"

# Install the temp directory and run the script
echo "Running setup script..."
cd "$TEMP_DIR"

# Create a wrapper script to run the Node.js script
cat > "$TEMP_DIR/run.js" << 'EOF'
#!/usr/bin/env node
const fs = require('fs');
const path = require('path');
...
// Check if we're in the temp directory
if (targetDir.includes('tmp')) {
  console.log("Determining target directory...");
  ...
  rl.question('Enter the path to your project directory: ', (projectDir) => {
    rl.close();
    ...
    const fullPath = path.resolve(normalizedDir);
    
    if (!fs.existsSync(fullPath)) {
      console.log(`Directory ${fullPath} does not exist. Creating it...`);
      try {
    ...
    console.log(`Running setup in: ${fullPath}`);
    
    // Copy the setup script to the target directory
    const setupScript = path.join(__dirname, 'setup-stackbit.js');
    const targetScript = path.join(fullPath, 'setup-stackbit.js');
    
    fs.copyFileSync(setupScript, targetScript);
    
    // Change to the target directory and run the script
    process.chdir(fullPath);
    
    const nodeProcess = spawn('node', [targetScript], {
      stdio: 'inherit',
      shell: true
    });
    
    nodeProcess.on('close', (code) => {
      // Clean up the script after it runs
      try {
        fs.unlinkSync(targetScript);
      } catch (error) {
        console.warn(`Warning: Could not remove temporary script: ${error.message}`);
      }
      
      if (code === 0) {
        console.log(`\n${'-'.repeat(50)}`);
        console.log(`Setup completed successfully in: ${fullPath}`);
      } else {
  ...
  // We're already in a project directory, just run the setup
  console.log(`Running setup in current directory: ${targetDir}`);
  
  const nodeProcess = spawn('node', ['setup-stackbit.js'], {
    stdio: 'inherit',
    shell: true
  });
  ...
  nodeProcess.on('close', (code) => {
    // Clean up the script after it runs
    try {
      fs.unlinkSync('setup-stackbit.js');
    } catch (error) {
      console.warn(`Warning: Could not remove temporary script: ${error.message}`);
    }
    ...
    if (code === 0) {
      console.log(`\n${'-'.repeat(50)}`);
      console.log(`Setup completed successfully in: ${targetDir}`);
    } else {
      console.error(`\nSetup failed with code: ${code}`);
    }
...
echo ""
echo "Your website is now configured for visual editing with Stackbit, Netlify, and GitHub continuous deployment."
